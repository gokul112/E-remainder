...
while ($row=mysql_fetch_array($result)){
	...
	$recurring=$row['recurring'];
        ...
        $appear_as_email=$row['appear_as_email'];
	...
	/* E-mail reminder */
        ...
        // If user just wanted the reminder to look like a basic email, just mail the message.

        if ($appear_as_email=="yes") {
           $from = $from . $hostmask;
           mail ("$email","$subject","$comment", "From: $from");
        } else {

          // Message with E*Reminder text.
          if ($adv_notice=="") {
		$formdate="[".$subject."]";
		$comment=$comment.$addcomment;                
		mail ("$email","$mail_erem $formdate","$comment", "From: $from");

          // Advance Message with E*Reminder text.
          } else {
